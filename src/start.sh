...
then
    ...
    su -w "$environment" -c "weston --shell=kiosk-shell.so -- /usr/src/app/startmpv.sh" - chromium
else
    echo "Not a video, launching Chromium"
    su -w "$environment" -c "weston --shell=kiosk-shell.so -- /usr/src/app/startwayland.sh" - chromium
fi
