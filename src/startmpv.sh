...
if [ -z ${SHOW_DEVICE_TAG+x} ] || [ "$SHOW_DEVICE_TAG" != "0" ]; then
  ...
  MPV_OSD_CONFIG="--osd-msg1=$BALENA_APP_NAME/$BALENA_DEVICE_NAME_AT_INIT --osd-align-x=left --osd-align-y=bottom --osd-margin-x=0 --osd-margin-y=0 --osd-font-size=12 --osd-back-color="#180048" --osd-color="#60F0F8" --osd-border-size=0"
  echo $MPV_OSD_CONFIG
else
